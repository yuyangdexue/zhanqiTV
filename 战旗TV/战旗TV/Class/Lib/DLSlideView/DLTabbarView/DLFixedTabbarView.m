...
- (void)layoutTabbar{
    ...
    for (NSInteger i=0; i<self.tabbarItems.count; i++) {
        ...
        imageView.frame = CGRectMake( x + (width-imageView.bounds.size.width-CGRectGetWidth(label.bounds))/2.0f, (height-imageView.bounds.size.height)/2.0, CGRectGetWidth(imageView.bounds), CGRectGetHeight(imageView.bounds));
        selectedIamgeView.frame = imageView.frame;
              label.frame = CGRectMake(  imageView.frame.origin.x + imageView.bounds.size.width+kImageSpacingX, (height-label.bounds.size.height)/2.0f, CGRectGetWidth(label.bounds), CGRectGetHeight(label.bounds));
    }
