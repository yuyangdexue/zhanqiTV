...
#define kSVPullAnimatedDuration 0.3
...
@property(nonatomic, assign) BOOL isDidHandler;
...
- (void)layoutSubviews {
  //  NSLog(@"layoutSubviews===> SVPull2Refresh ");
  float position = 0.50;
  CGPoint superCenter =
      CGPointMake(self.superview.bounds.size.width * position,
                  self.superview.bounds.size.height * position);

  CGRect titleFrame = self.titleLabel.frame;
  CGFloat remainingWidth =
      self.superview.bounds.size.width - titleFrame.size.width;

  titleFrame.origin.x = ceilf(superCenter.x - titleFrame.size.width / 2);
  titleFrame.origin.y =
  ...
  subtitleFrame.origin.x = ceilf(superCenter.x - subtitleFrame.size.width / 2);
...
- (SVPullToRefreshArrow *)arrow {
  if (!_arrow) {
    _arrow = [[SVPullToRefreshArrow alloc]
        initWithFrame:CGRectMake(0, (self.bounds.size.height - 30) / 2, 30,
                                 30)];
...
- (UILabel *)titleLabel {
  if (!_titleLabel) {
    _titleLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, 190, 20)];
...
- (UILabel *)subtitleLabel {
  if (!_subtitleLabel) {
    _subtitleLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, 190, 20)];
...
- (UIColor *)arrowColor {
  return self.arrow.arrowColor; // pass through
                                // return self.arrow.backgroundColor;
}
...
- (void)setArrowColor:(UIColor *)newArrowColor {
  ...
  //  self.arrow.backgroundColor = newArrowColor;
}
...
- (void)startAnimating {
  if (fequalzero(self.scrollView.contentOffset.y)) {
    ...
    [UIView animateWithDuration:kSVPullAnimatedDuration
        animations:^{
          [self.scrollView
  ...
  if (!self.wasTriggeredByUser &&
    ...
    [self.scrollView
        setContentOffset:CGPointMake(self.scrollView.contentOffset.x,
                ...
                animated:YES];
    //    NSLog(@"stopAnimating===>2");
  }
}

- (void)stopAnimatingInstant {

  //  NSLog(@"stopAnimating===>1 instant");
  CGPoint newPOS =
      CGPointMake(self.scrollView.contentOffset.x, -self.originalTopInset);
  [UIView animateWithDuration:kSVPullAnimatedDuration
      animations:^{
        self.scrollView.contentOffset = newPOS;
      }
      completion:^(BOOL finished) {
        _state = SVPullToRefreshStateStopped;
        [self setNeedsLayout];
        //          NSLog(@"stopAnimating===>2 instant");
      }];
}
...
- (void)setState:(SVPullToRefreshState)newState {
  ...
  case SVPullToRefreshStateLoading:
    ...
    if (previousState == SVPullToRefreshStateTriggered) {
      NSLog(@"handler ==== 2 %d", self.wasTriggeredByUser);
      [self performSelector:@selector(performHandler)
                 withObject:nil
                 afterDelay:kSVPullAnimatedDuration];
    }
    break;
  }
}
...
- (void)drawRect:(CGRect)rect {
  ...
  CGPoint center = CGPointMake(rect.size.width / 2,
                               rect.size.height / 2); // get the circle centre
  CGFloat radius = 0.6 * center.x;                    // little scaling needed
  CGFloat startAngle = -((float)M_PI / 2);            // 90 degrees
  CGFloat endAngle = ((2 * (float)M_PI) + startAngle);
  CGContextAddArc(ctx, center.x, center.y, radius + 4, startAngle, endAngle, 0);
  CGContextStrokePath(ctx); // draw

  CGFloat horn = 7.0;

  // the arrow
  CGContextMoveToPoint(ctx, center.x, horn);
  CGContextAddLineToPoint(ctx, center.x, center.y * 2 - horn + 0.5);
  CGContextStrokePath(ctx); // draw

  CGContextMoveToPoint(ctx, center.x, center.y * 2 - horn);
  CGContextAddLineToPoint(ctx, center.x - horn, center.y * 2 - horn * 2);
  CGContextStrokePath(ctx); // draw
